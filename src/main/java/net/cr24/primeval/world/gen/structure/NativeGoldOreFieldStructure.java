...
public class NativeGoldOreFieldStructure extends Structure {
    ...
    public static final MapCodec<NativeGoldOreFieldStructure> CODEC = createCodec(NativeGoldOreFieldStructure::new);
    ...
    private static void addPieces(StructurePiecesCollector collector, Structure.Context context) {
        ...
        if (context.biomeSource().getBiome(context.chunkPos().getStartX(), 100, context.chunkPos().getStartZ(), context.noiseConfig().getMultiNoiseSampler()).isIn(PrimevalTags.Biomes.RAISED_ORES)) {
            height = context.random().nextBetween(-40, -30);
        } else {
            height = context.random().nextBetween(-60, -50);
        }
