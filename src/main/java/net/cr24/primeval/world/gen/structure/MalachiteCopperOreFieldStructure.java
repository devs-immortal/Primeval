...
public class MalachiteCopperOreFieldStructure extends Structure {
    ...
    public static final MapCodec<MalachiteCopperOreFieldStructure> CODEC = createCodec(MalachiteCopperOreFieldStructure::new);
    ...
    private static void addPieces(StructurePiecesCollector collector, Structure.Context context) {
        ...
        if (context.biomeSource().getBiome(context.chunkPos().getStartX(), 100, context.chunkPos().getStartZ(), context.noiseConfig().getMultiNoiseSampler()).isIn(PrimevalTags.Biomes.RAISED_ORES)) {
            height = context.random().nextBetween(-15, 0);
        } else {
            height = context.random().nextBetween(-50, -25);
        }
